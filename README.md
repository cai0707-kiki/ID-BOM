...
- 🔌 **线缆筛选** — 按长度和材质快速筛选线缆配件
- 🖼️ **图片预览与缓存** — 配件图片支持点击放大查看，Service Worker 实现本地持久化缓存
...
- `index.html` — 配单工具
  - 新增「系列表」页面：按产品系列分组浏览全部相机及关联配件，左侧系列导航 + 右侧详情表格
  - 新增返回顶部按钮：滚动超过 300px 自动显示，点击平滑回到页面顶部
  - 优化扩展配件类型显示：配单明细中扩展配件不再笼统显示"扩展配件"，而是显示具体子类别名称（如 IO-BOX、BZ3 等）

### V1.2.1 (2026-08-19)
**公告系统增强 & 图片缓存**

- `index.html` — 配单工具
  - 新增公告浮动按钮：用户关闭每日自动弹窗后，仍可随时点击查阅公告内容
